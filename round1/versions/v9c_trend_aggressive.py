    ...
    def run(self, state: TradingState) -> tuple:
        ...
        for product in state.order_depths:
            ...
            if product == 'INTARIAN_PEPPER_ROOT':
                ...
                # Ramp to max at trend=3 (was 5), max bias ±5 (was ±3)
                trend_norm = max(-1.0, min(1.0, trend / 3.0))
                trend_bias = trend_norm * 7.0

            # ── Adjusted fair price ──
            imbalance_weight = 1.0
            # Pepper: lighter skew to allow trend positions
            # Osmium: strong skew (unchanged from V3)
            skew_weight = 1.5 if product == 'INTARIAN_PEPPER_ROOT' else 3.0

...
class Trader:
    """
    Round 3 V5 – Selective Taking + Delta Hedging
    ===============================================
    V3 baseline: +1803. V4 wider spreads: +1733 (no help).
    
    V5 changes:
      1. SELECTIVE TAKING: only take when edge > threshold
         Filters out adverse selection from marginal spikes
      2. DELTA HEDGING: offset VEV delta exposure with VE
         Isolates MM spread profit from directional risk
      3. HYDROGEL: keep (free +650) with V3 spreads (V4 was worse)
      4. Deep ITM VEVs: take when mispriced vs intrinsic
    """
    ...
    def run(self, state: TradingState) -> tuple:
        ...
        sigma_ema = memory.get('sigma_ema', 0.24)
        ...
        # ── Calculate total VEV delta for hedging ──
        total_vev_delta = 0.0
        for vev_name, K in self.VEV_STRIKES.items():
            pos = state.position.get(vev_name, 0)
            if pos != 0:
                d = self.bs_delta(ve_fair, K, T, sigma)
                total_vev_delta += pos * d
        ...
        for product in state.order_depths:
            ...
            if is_vev:
                ...
                # TAKING THRESHOLD for VEVs: only take with clear edge
                take_threshold = 2.0
                ...
                take_threshold = 0  # no taking for hydrogel
                ...
                take_threshold = 1.5  # selective taking on VE
            ...
            # ── DELTA HEDGING BIAS for VE ──
            # If we have net long VEV delta, we want to be SHORT VE to hedge
            # Bias adj_fair DOWN → sell more VE → go short
            if is_ve and abs(total_vev_delta) > 2:
                # We want VE position = -total_vev_delta
                target_ve_pos = -total_vev_delta
                hedge_gap = target_ve_pos - position  # positive = need to buy
                # Shift fair to encourage trading toward target
                hedge_bias = max(-3.0, min(3.0, hedge_gap * 0.1))
                adj_fair += hedge_bias
            ...
            # ──────────────────────────────────────────────────
            # LAYER 1: SELECTIVE TAKING
            # Only take when edge > threshold (filters adverse selection)
            # ──────────────────────────────────────────────────
            if take_threshold > 0:
                for ask_price in sorted(order_depth.sell_orders.keys()):
                    if ask_price >= adj_fair - take_threshold:
                        break
                    if buy_budget <= 0:
                        break
                ...
                for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                    if bid_price <= adj_fair + take_threshold:
                        break
                    if sell_budget <= 0:
                        break

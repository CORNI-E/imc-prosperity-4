...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple:
        ...
        for product in state.order_depths:
            ...
            # Aggressive trend capture WITH safety nets
            trend_bias = 0.0
            ...
            if product == 'INTARIAN_PEPPER_ROOT':
                ...
                # SAFETY NET: trend confidence = |trend| / volatility
                # High = smooth trend (real), Low = noisy (false signal)
                if volatility > 0.1:
                    trend_strength = abs(trend) / volatility
                else:
                    trend_strength = abs(trend)  # low vol = strong signal
                
                # Confidence ramps from 0 to 1 as trend_strength goes 0→2
                trend_confidence = max(0.0, min(1.0, trend_strength / 2.0))
                
                # Full bias (×7) only when confident, mild (×1.4) when unsure

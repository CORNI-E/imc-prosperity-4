...
class Trader:
    ...
    VEV_EFFECTIVE_LIMIT = 200  # increased for more ITM capacity
    ...
    def run(self, state: TradingState) -> tuple:
        ...
        ema_slow = memory.get('ema_slow', {})  # EMA of implied vol
        ...
        for product in state.order_depths:
            ...
            # Defaults for trend variables (only HYDROGEL overrides these)
            trend_bias = 0.0
            ...
            if is_vev:
                ...
                # SKIP losing/zero VEVs: 5200+ lose or make 0
                if K >= 5200:
                    result[product] = orders
                    continue
                ...
                if K <= 4500:
                    ...
                    base_spread = 1.5  # slightly wider for ATM
                ...
                # ── Slow EMA for trend detection (V9d approach) ──
                alpha_slow_val = 0.02
                ...
                # ── Trend bias with confidence gate ──
                trend = fair - ema_slow[product]
                trend_norm = max(-1.0, min(1.0, trend / 3.0))
                ...
                trend_confidence = max(0.0, min(1.0, trend_strength / 2.0))
                trend_bias = trend_norm * 7.0 * trend_confidence
                ...
                # ── Trend bias (reduced ×5 for tighter VE spread) ──
                trend = fair - ema_slow[product]
                trend_norm = max(-1.0, min(1.0, trend / 3.0))
                if volatility > 0.1:
                    trend_strength = abs(trend) / volatility
                    ...
                    trend_strength = abs(trend)
                trend_confidence = max(0.0, min(1.0, trend_strength / 2.0))
                trend_bias = trend_norm * 5.0 * trend_confidence

            # ── Market signals ──
            imbalance = (vol_bid - vol_ask) / total_vol if total_vol > 0 else 0
            inv_ratio = position / limit if limit > 0 else 0

            # Product-specific skew
            if is_hydrogel or is_ve:
                # Adaptive skew: light when trend is confident, heavy when unsure
                skew_weight = 3.0 - trend_confidence * 1.5  # 3.0 → 1.5
            elif is_vev:
                skew_weight = 2.0
            else:
                skew_weight = 3.0
            ...
            # Add trend bias for HYDROGEL and VE
            adj_fair = fair + (imbalance * 1.0) - (exp_skew * skew_weight) + trend_bias

            buy_budget = limit - position
            sell_budget = position + limit

            # ──────────────────────────────────────────────────
            # LAYER 1: TAKING - DISABLED
            # R1 + R3 analysis: spike bots are INFORMED
            # Taking their spikes = negative EV on ALL products
            # Pure MM posting only
            # ──────────────────────────────────────────────────
            ...
            buy_size_1 = int(buy_budget * 0.85)
            sell_size_1 = int(sell_budget * 0.85)
